...
function getrestaurant(x){
//x is like[{},{},{}..]
    const root = document.getElementById('root');
    
    x.forEach(restaurant => {
        
    //   create a card <div class='card'></div>
    ...
    // Card header
       ...
       Card_header.appendChild(rate);//can i use only append
   ...
   Card_content.append(Card_header,Card_footer,card_location,"text");
...
getrestaurant(restaurants);//starting all restaurants

//1.alcohol button to show specific restaurants using getrestaurants
document.getElementById("Alcohol").addEventListener('click',()=>{
    
    const result = restaurants.filter((obj)=> obj.alcohol);
    document.getElementById('root').replaceChildren();   //better for removing ,do not innerHTML =" "
    getrestaurant(result); //dont use .remove() bcs root bhi chala jayega

})

//2.rating button to show specific restaurants using getrestaurants
document.getElementById("Rating").addEventListener('click',()=>{
    
    const result = restaurants.filter((obj)=>obj.rating>4.5); //returns when true
    document.getElementById('root').replaceChildren();
    getrestaurant(result);
 
})

//when press button sort(filters) square pop ups
document.getElementById('Filters').addEventListener('click',()=>{
    document.getElementById("filterPopup").classList.remove("hidden");
}) //classlist.add and .remove  helps in class attribute
// .hidden{display:none;}



//inside pop up applying
document.getElementById('applyFilter').addEventListener('click',()=>{
   
    const element = document.querySelector('input[name="filterOption"]:checked');
    //used for selectede radio buttons
    const answer = element.value;

    if(answer==="rating"){
        
        //these 4 are taking default restraunts user experience bad
